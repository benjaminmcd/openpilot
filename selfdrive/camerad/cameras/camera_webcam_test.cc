...
namespace
{
  ...
  void run_camera(CameraState *s, cv::Mat *frame, float *ts, cv::Size *size, cv::Mat *transform, uint32_t *frame_id, size_t *buf_idx)
  {
    ...
    cv::warpPerspective(*frame, transformed_mat, *transform, *size, cv::INTER_LINEAR, cv::BORDER_CONSTANT, 0);
  ...
  static void road_camera_thread(CameraState *s)
  {
    ...
    cap_road.read(capture_frame);

    while(!do_exit){
      cap_road.read(capture_frame);

      run_camera(s, &capture_frame, ts, &size, &transform, &frame_id, &buf_idx);
    }
  }
